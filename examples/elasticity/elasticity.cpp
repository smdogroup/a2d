...
int main( int argc, char *argv[] ){
  ...
  MultiArray<ScalarType, CLayout<nodes_per_elem, 3> > Pe(node_element_layout, Pe_data);
  ...
  ScalarType* jac_data = new ScalarType[ nelems * nodes_per_elem * nodes_per_elem * 3 * 3 ];
  ...
  for ( int i = 0; i < 10; i++ ){
    for ( int jy = 0; jy < nodes_per_elem; jy++ ){
      for ( int iy = 0; iy < 3; iy++ ){
        ...
        std::cout << "fd: " << std::setw(20) << fd << " result: " << std::setw(20) << result.real()
                  << " error: " << std::setw(20) << (fd - result.real())/fd << std::endl;
      }
    }
  }
