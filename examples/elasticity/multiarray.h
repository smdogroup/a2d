...
*/
template <int r, size_t... dims>
struct ___get_extent;

template <size_t dim0, size_t... dims>
struct ___get_extent<0, dim0, dims...> {
  static const size_t extent = dim0;
};

template <int r, size_t dim0, size_t... dims>
struct ___get_extent<r, dim0, dims...> {
  static const size_t extent = ___get_extent<r-1, dims...>::extent;
};
...
class FLayout {
public:
  FLayout( const size_t dim1 ) : dim1(dim1), static_extents{dims...} {}
  const size_t dim1;
  static const size_t rank = sizeof... (dims) + 1;

  static const size_t get_rank(){
    return rank;
  }

  const size_t get_extent( size_t index ) const {
    if (index == 0){
      return dim1;
    }
  ...
  const size_t compute_index( Idx i1, IdxType... idx ) const {
    return i1 + dim1 * __compute_index<0>(idx...);
  }

private:
  size_t static_extents[rank-1];

  template<int r, class Idx, class... IdxType>
  static const size_t __compute_index( Idx i, IdxType... idx ){
    return i + ___get_extent<r, dims...>::extent * __compute_index<r+1>(idx...);
  }

  template<int r, class Idx>
  static const size_t __compute_index( Idx i ){
    return i;
  }
};
...
class CLayout {
public:
  CLayout( const size_t dim1 ) : dim1(dim1), static_extents{dims...} {}
  const size_t dim1;
  static const size_t rank = sizeof... (dims) + 1;
...
private:
  ...
  template<int r, class Idx, class... IdxType>
  static const size_t __compute_index( const size_t index, Idx i, IdxType... idx ){
    return __compute_index<r+1>(index * ___get_extent<r, dims...>::extent + i, idx...);
  }

  template<int r, class Idx>
  static const size_t __compute_index( const size_t index, Idx i ){
...
public:
  ...
  const size_t extent( size_t index ) const {
