...
class NonlinearElasticity3D {
 public:
  ...
  template <typename T, class IdxType, class QuadPointData>
  static T compute_residual(IdxType i, IdxType j, QuadPointData& data, T wdetJ,
  ...
  template <typename T, class IdxType, class QuadPointData>
  static T compute_jacobian(IdxType i, IdxType j, QuadPointData& data, T wdetJ,
                            A2D::Mat<T, 3, 3>& Jinv, A2D::Mat<T, 3, 3>& Uxi0,
                            A2D::Mat<T, 3, 3>& Uxib,
  ...
  static void energy(QuadPointModelDataArray& Edata, QuadPointDetJArray& detJ,
    ...
    for (std::size_t j = 0; j < Quadrature::getNumQuadPoints(); j++) {
      ...
      for (std::size_t i = 0; i < Uxi.extent(0); i++) {
        ...
        energy += weight * detJ(i, j) *
                  Model::compute_energy(i, j, Edata, Jinv0, Uxi0);
      }
    }
  }
  ...
  static void residuals(QuadPointModelDataArray& Edata,
    ...
    for (std::size_t j = 0; j < Quadrature::getNumQuadPoints(); j++) {
      ...
      for (std::size_t i = 0; i < Uxi.extent(0); i++) {
        ...
        Model::compute_residual(i, j, Edata, weight * detJ(i, j), Jinv0, Uxi0,
                                Uxib);
