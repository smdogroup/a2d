...
int main(int argc, char *argv[]) {
  Kokkos::initialize(argc, argv);
  {
    ...
    const index_t num_traction_verts = (ny + 1) / 4;
    ...
    for (index_t j = 0; j < (ny + 1) / 4; j++) {
      boundary_verts[j] = node_num(nx, j);
    }
    ...
    StopWatch watch;
    double t1 = watch.lap();
    TopoElasticityAnalysis2D<T> prob(conn, bcinfo, nquad, quad.data(),
                                     Xloc.data(), traction_label, tx_traction);
    double t2 = watch.lap();
