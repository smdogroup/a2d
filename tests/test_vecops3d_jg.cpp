...
protected:
    // Input Options:
    const T x_data[3] = {0.20970639762168464, 0.10308079207850707, 0.9173999579053238};
    const T a_data = 0.6896026553439034;
    const T v_data[3] = {0.4242524109372384, 0.28692392009681067, 0.9492874123274755};
    const T y_data[3] = {0.6854063555079019, 0.7322747860081501, 0.3571801526014854};
    const T S_data[9] = {0.562738676354188, 0.7541181002046244, 0.5766762373185819, 
0.7541181002046244, 0.0011522173917457579, 1.4376110744805093, 
0.5766762373185819, 1.4376110744805093, 0.5936808838281609};
    const T xb_data[3] = {0.9028664628429675, 0.503710207783976, 0.715447313858641};
    const T ab_data = 0.2434746529642985;
    const T vb_data[3] = {0.7366391012272651, 0.6723153242502288, 0.22166250342375682};
    const T yb_data[3] = {0.9537280081498468, 0.7756965964838682, 0.7088390653667941};
    const T Sb_data[9] = {1.7739061006868846, 1.046007355988558, 0.9127544973325203, 
...
protected:
    ...
    const T xb_out[3] = {0.053933286154386834, 0.02651080710575372, 0.23594127317465283};
};  /*UNQ_T1F_FTV_01*/
...
protected:
    // Results
    const T v_out[3] = {0.14461408864251815, 0.07108478793229128, 0.6326414469838965};
};  /*UNQ_T1F_FTV_01*/
...
protected:
    // Results
    const T v_out[3] = {0.14461408864251815, 0.07108478793229128, 0.6326414469838965};
    const T vb_out[3] = {0.6226191101974682, 0.3473598968116592, 0.493374367395582};
    const T xb_out[3] = {0.5079882802364685, 0.4636304328313552, 0.15285905095119978};
};  /*UNQ_T1F_FTV_01*/
...
protected:
    // Results
    const T v_out[3] = {0.14461408864251815, 0.07108478793229128, 0.6326414469838965};
    const T vb_out[3] = {0.05105819238533285, 0.025097560078599514, 0.22336363638046075};
...
protected:
    // Results
    const T v_out[3] = {0.14461408864251815, 0.07108478793229128, 0.6326414469838965};
    const T vb_out[3] = {0.673677302582801, 0.3724574568902587, 0.7167380037760427};
    const T xb_out[3] = {0.5079882802364685, 0.4636304328313552, 0.15285905095119978};
    const T ab_out = 0.42713389972602034;
};  /*UNQ_T1F_FTV_01*/
...
protected:
    ...
    const T xb_out[3] = {0.166879074546811, 0.17829034939784028, 0.08696431370038184};
};  /*UNQ_T1F_FTV_01*/
...
protected:
    ...
    const T xb_out[3] = {0.166879074546811, 0.17829034939784028, 0.08696431370038184};
    const T yb_out[3] = {0.05105819238533286, 0.025097560078599517, 0.22336363638046075};
};  /*UNQ_T1F_FTV_01*/
...
protected:
    // Results
    const T v_out[3] = {0.22151499344079667, 0.10888528552350413, 0.9690588745155729};
};  /*UNQ_T1F_FTV_01*/
...
protected:
    // Results
    const T v_out[3] = {0.22151499344079667, 0.10888528552350413, 0.9690588745155729};
    const T vb_out[3] = {0.7318494451936044, 0.42302052906313753, -0.21482320797600826};
    const T xb_out[3] = {0.67254703009868, 0.6582796133772945, -0.22770155706806752};
};  /*UNQ_T1F_FTV_01*/
...
protected:
    // Results
    const T S_out[9] = {0.030326499574565464, 0.014906934802982205, 0.13266895883316293, 
...
0.014906934802982205, 0.007327476244793286, 0.06521318145679289, 
0.13266895883316293, 0.06521318145679289, 0.5803852368321898};
    const T Sb_out[9] = {0.010707229594203083, 0.005263118913176909, 0.046840783545026285, 
...
TEST_F(Vec3ScaleSymmetricOuterProduct_ADScalarVec, forward) {
    ...
    // Evaluations:
    auto expr = A2D::Vec3ScaleSymmetricOuterProduct(a_ad, x, S_ad);  /*UNQ_T1F_TFF_04*/
    expr.forward();  /*UNQ_T1F_TFF_05*/
    // Comparisons:
...
TEST_F(Vec3ScaleSymmetricOuterProduct_ADScalarVec, reverse) {
    ...
    // Evaluations:
    auto expr = A2D::Vec3ScaleSymmetricOuterProduct(a_ad, x, S_ad);  /*UNQ_T1F_TFR_04*/
    expr.reverse();  /*UNQ_T1F_TFR_05*/
    // Comparisons:
...
0.014906934802982205, 0.007327476244793286, 0.06521318145679289, 
0.13266895883316293, 0.06521318145679289, 0.5803852368321898};
    const T Sb_out[9] = {0.2611344213798594, 0.13702366368098343, 0.6746545067516124, 
0.13702366368098343, 0.07161226659930862, 0.36952537529538265, 
0.6746545067516124, 0.36952537529538265, 0.9052432477605453};
    const T xb_out[3] = {1.8166667021956102, 0.714766963705857, 1.3831101199754792};
};  /*UNQ_T1F_FTV_01*/
...
TEST_F(Vec3ScaleSymmetricOuterProduct_ScalarADVec, forward) {
    ...
    ADMat_t S_ad(S, Sb);  /*UNQ_T1F_TFF_03*/
    // Evaluations:
    auto expr = A2D::Vec3ScaleSymmetricOuterProduct(a, x_ad, S_ad);  /*UNQ_T1F_TFF_04*/
    expr.forward();  /*UNQ_T1F_TFF_05*/
    // Comparisons:
    expect_mat_eq<3, 3>(S_ad.value(), S_out);  /*UNQ_T1F_TFF_06*/
    expect_mat_eq<3, 3>(S_ad.bvalue(), Sb_out);  /*UNQ_T1F_TFF_07*/
...
TEST_F(Vec3ScaleSymmetricOuterProduct_ScalarADVec, reverse) {
    ...
    ADMat_t S_ad(S, Sb);  /*UNQ_T1F_TFR_03*/
    // Evaluations:
    auto expr = A2D::Vec3ScaleSymmetricOuterProduct(a, x_ad, S_ad);  /*UNQ_T1F_TFR_04*/
    expr.reverse();  /*UNQ_T1F_TFR_05*/
    // Comparisons:
    expect_mat_eq<3, 3>(S_ad.value(), S_out);  /*UNQ_T1F_TFR_06*/
    expect_vec_eq<3>(x_ad.bvalue(), xb_out);  /*UNQ_T1F_TFR_07*/
...
0.014906934802982205, 0.007327476244793286, 0.06521318145679289, 
0.13266895883316293, 0.06521318145679289, 0.5803852368321898};
    const T Sb_out[9] = {0.27184165097406254, 0.14228678259416036, 0.7214952902966387, 
...
TEST_F(Vec3ScaleSymmetricOuterProduct_ADScalarADVec, forward) {
    ...
    ADMat_t S_ad(S, Sb);  /*UNQ_T1F_TFF_03*/
    // Evaluations:
    auto expr = A2D::Vec3ScaleSymmetricOuterProduct(a_ad, x_ad, S_ad);  /*UNQ_T1F_TFF_04*/
    expr.forward();  /*UNQ_T1F_TFF_05*/
    // Comparisons:
    expect_mat_eq<3, 3>(S_ad.value(), S_out);  /*UNQ_T1F_TFF_06*/
    expect_mat_eq<3, 3>(S_ad.bvalue(), Sb_out);  /*UNQ_T1F_TFF_07*/
...
TEST_F(Vec3ScaleSymmetricOuterProduct_ADScalarADVec, reverse) {
    ...
    ADMat_t S_ad(S, Sb);  /*UNQ_T1F_TFR_03*/
    // Evaluations:
    auto expr = A2D::Vec3ScaleSymmetricOuterProduct(a_ad, x_ad, S_ad);  /*UNQ_T1F_TFR_04*/
    expr.reverse();  /*UNQ_T1F_TFR_05*/
    // Comparisons:
    expect_mat_eq<3, 3>(S_ad.value(), S_out);  /*UNQ_T1F_TFR_06*/
    expect_val_eq(a_ad.bvalue, ab_out);  /*UNQ_T1F_TFR_07*/
